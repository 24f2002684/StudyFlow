  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div
          {...dragHandleProps}
          className={`cursor-grab active:cursor-grabbing text-neutral-300 dark:text-[#444444] group-hover:text-neutral-500 dark:group-hover:text-[#777777] p-1 -ml-1 transition-colors ${
        ...
        <button
          ...
          className={`relative mt-0.5 w-5 h-5 rounded-lg flex items-center justify-center border transition-all duration-200 active:scale-90 cursor-pointer shrink-0 ${
            task.completed
              ? 'bg-[#2dd4bf] border-[#2dd4bf] text-neutral-900 shadow-xs'
              : 'border-neutral-300 dark:border-[#333333] hover:border-[#ff4d6d] dark:hover:border-[#ff4d6d] bg-neutral-50 dark:bg-[#0f0f0f]'
          }`}
        >
          {task.completed && (
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            ...
            >
              {!task.completed && (
                <button
                  onClick={() => onEdit(task)}
                  className="p-1.5 rounded-lg text-neutral-400 hover:text-neutral-700 dark:text-[#666666] dark:hover:text-[#e5e5e5] hover:bg-neutral-100 dark:hover:bg-[#1f1f1f] transition-colors"
                  title="Edit task"
                >
              ...
              <button
                ...
                disabled={isDeleting}
                className="p-1.5 rounded-lg text-neutral-400 hover:text-[#ff4d6d] dark:text-[#666666] dark:hover:text-[#ff4d6d] hover:bg-neutral-100 dark:hover:bg-[#1f1f1f] transition-colors"
                title="Delete task"
              >
          ...
          <div className="flex flex-wrap items-center gap-2 mt-2 text-xs">
            {/* Subject / Course Tag */}
            {task.subject && (
              <span className="inline-flex items-center space-x-1.5 px-2.5 py-0.5 rounded-md bg-neutral-100 dark:bg-[#1a1a1a] text-neutral-700 dark:text-[#d4d4d4] font-medium border border-neutral-200 dark:border-[#262626]">
            ...
            {task.dueDate && (
              <span
                className={`inline-flex items-center space-x-1 px-2 py-0.5 rounded-md font-medium text-[11px] ${
                  dueInfo.isOverdue
            ...
            {task.pomodorosSpent > 0 && (
              <span
                className="inline-flex items-center space-x-1 px-2 py-0.5 rounded-md bg-neutral-100 dark:bg-[#1a1a1a] text-[#ff4d6d] font-semibold text-[11px] border border-neutral-200 dark:border-[#262626]"
